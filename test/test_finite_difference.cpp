...
TEST(FiniteDifference, Univariate)
{
    ...
    auto f = [](const Matrix<1>& x) { return Matrix<1>{std::exp(x(0))}; };
    ...
    auto g = [](const Matrix<1>& x) { return Matrix<1>{std::sin(x(0))}; };
...
TEST(FiniteDifference, Multivariate)
{
    ...
    auto f = [](const Matrix<3>& x) { return Matrix<1>{5.0 * x(0) * x(0) + 3.5 * x(1) * x(2) + 2.0 * x(0) - x(2)}; };
